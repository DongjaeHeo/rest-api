...
class LinkTagsToItem(MethodView):
    ...
    def post(self, item_id, tag_id):
        ...
        if item.store_id != tag.store_id:
            abort(
                400,
                message="The item and the tag must belong to the same store.",
            )
...
class Tag(MethodView):
    ...
    @blp.alt_response(
        400,
        description="Returned if the tag is assigned to on or more items. In this case, the tag is not deleted."
    )
    def delete(self, tag_id):
        ...
        abort(
            400,
            message="Could not delete tag. There are still items assigned to it."
        )
